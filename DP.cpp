...
static int* N_RESProblem(unsigned int* p, unsigned int* w,const int len, const unsigned int s, const unsigned int t){
     ...
     Mat[0][0] = 0;
     for (int i = 0; i < len - 1; i++) Mat[0][0] += p[i]*w[i];
     ...
     int * x = new int[len];
     long min = Mat[len - 1][0];
     int minIndex = 0;
     for (int i = 1; i < s + 1; i++) {
         if(min > Mat[len - 1][i] && Mat[len - 1][i] != -1){
             min =  Mat[len - 1][i];
             minIndex = i;
         }
     }
...
int main(){
    ...
    unsigned int _t=5;
    unsigned int _s=2;
    int * _x = N_RESProblem(_p, _w, _n, _s, _t);
    for (int i = 0; i < _n; ++i) {
        cout << "  "<<_x[i];
    }
    cout << "\n";
